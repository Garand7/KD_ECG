...
import os
import numpy as np
from unet import unet_m
# from unet_s_at_exp2 import unet_s
# from unet_s_final import unet_s
# from unet_s_dfscrd import unet_s
from unet_s_at_final import unet_s
import pandas as pd
...
def make_args():
    ...
    parser.add_argument('--n_epoch_s', type=int, default=4500, help='architecture name')
    ...
    parser.add_argument('--save_dir', type=str, default='output_m_at', help='architecture name')
    parser.add_argument('--save_dir_at', type=str, default='output_s_at_75k', help='architecture name') #output_s_at
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    parser.add_argument('--lr_at', type=float, default=2e-4, help='learning rate')
...
def plot_loss_and_acc(train_loss, train_acc, val_loss, val_acc,dir):
    ...
    # plot train accuracy for each class
    # for i in range(3):
    #     axes[i + 2].plot(train_acc[:, i])
...
class Trainer:
    def __init__(self, args):
        ...
        os.makedirs(args.save_dir_at, exist_ok=True)
    ...
    def build_model(self):
        ...
        self.model_student = unet_s().to(device)
        ...
        self.optimizer_student = torch.optim.AdamW(self.model_student.parameters(), lr=self.args.lr_at)
    ...
    def distillation_at(self, T , alpha):
        ...
        for epoch in range(args.n_epoch_s):
            ...
            for step, (data, label) in enumerate(self.data_loader_train):
                ...
                loss_at = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_student/T, dim=1), F.softmax(output_teacher/T, dim=1))*T*T

                loss = alpha*loss_at+(1-alpha)*loss_ce
                train_loss_at.append(loss.item())
            ...
            logger.info(f'[{self.args.n_epoch_s}/{epoch}] <{len(self.data_loader_train)}/{step}>, loss: {loss.item()}')
            macc = acc_group / data_count
            train_acc_at.append(macc)

            if (epoch + 1) % self.args.val_freq == 0:
                acc, loss = self.val_student()
                # val_acc_at.append(np.sum(acc))
                val_acc_at.append(acc)
                val_loss_at.append(loss.item())
                if np.sum(acc) >= np.sum(self.best_acc):
                    self.best_acc = acc
                    torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_at, 'best_model.pth'))

            if (epoch + 1) % self.args.save_freq == 0:
                torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_at, f'{epoch}.pth'))
                # plot loss and accuracy
            plot_loss_and_acc(train_loss_at, train_acc_at, val_loss_at, val_acc_at, dir=args.save_dir_at)

            save_metrics_to_excel(train_loss_at, train_acc_at, val_loss_at, val_acc_at, self.args.save_dir_at)

if __name__ == '__main__':
    args=make_args()
    trainer=Trainer(args)
    trainer.train()
    trainer.distillation_at(T=3, alpha=0.6)