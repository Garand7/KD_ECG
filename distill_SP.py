...
def make_args():
    ...
    parser.add_argument('--save_dir', type=str, default='output_m_sp', help='architecture name')
    parser.add_argument('--save_dir_sp', type=str, default='output_s_sp_75k', help='architecture name') #output_s_sp_175
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    parser.add_argument('--lr_sp', type=float, default=3e-4, help='learning rate')
...
class Trainer:
    def __init__(self, args):
        ...
        os.makedirs(args.save_dir_sp, exist_ok=True)
    ...
    def build_model(self):
        ...
        self.optimizer_student = torch.optim.AdamW(self.model_student.parameters(), lr=self.args.lr_sp)
    ...
    def distillation_sp(self, T , alpha):
        self.best_acc = 0
        train_acc_sp =[]
        train_loss_sp = []
        val_acc_sp = []
        ...
        for epoch in range(args.n_epoch_s):
            ...
            for step, (data, label) in enumerate(self.data_loader_train):
                ...
                loss_sp = sp_distillation(output_student,output_teacher)

                loss_kd = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_student/T, dim=1), F.softmax(output_teacher/T, dim=1))*T*T

                loss = alpha*loss_sp + (1-alpha)*loss_ce
                train_loss_sp.append(loss.item())
            ...
            logger.info(f'[{self.args.n_epoch_s}/{epoch}] <{len(self.data_loader_train)}/{step}>, loss: {loss.item()}')
            macc = acc_group / data_count
            train_acc_sp.append(macc)

            if (epoch + 1) % self.args.val_freq == 0:
                acc, loss = self.val_student()
                val_acc_sp.append(acc)
                val_loss_sp.append(loss.item())
                if np.sum(acc) >= np.sum(self.best_acc):
                    self.best_acc = acc
                    torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_sp, 'best_model.pth'))

            if (epoch + 1) % self.args.save_freq == 0:
                torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_sp, f'{epoch}.pth'))
                # plot loss and accuracy
            plot_loss_and_acc(train_loss_sp, train_acc_sp,val_loss_sp, val_acc_sp,dir=args.save_dir_sp)

            save_metrics_to_excel(train_loss_sp, train_acc_sp,val_loss_sp, val_acc_sp, self.args.save_dir_sp)
...
if __name__ == '__main__':
    ...
    trainer.train()
    trainer.distillation_sp(T=3,alpha=0.5)