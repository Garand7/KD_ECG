...
def make_args():
    ...
    parser.add_argument('--save_dir', type=str, default='output_m_crd_self', help='architecture name')
    parser.add_argument('--save_dir_crd', type=str, default='output_s_crd_self_75k', help='architecture name') #output_s_crd_self_225
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    parser.add_argument('--lr_crd', type=float, default=2e-4, help='learning rate')
...
class Trainer:
    def __init__(self, args):
        ...
        os.makedirs(args.save_dir_crd, exist_ok=True)
    ...
    def build_model(self):
        self.model_teacher = unet_l(1, 3).to(device)
        ...
        self.optimizer_student = torch.optim.AdamW(self.model_student.parameters(), lr=self.args.lr_crd)
    ...
    def distillation_crd_self(self, T, alpha):
        self.best_acc = 0
        train_acc_crd =[]
        train_loss_crd = []
        val_acc_crd = []
        ...
        for epoch in range(args.n_epoch_s):
            ...
            for step, (data, label) in enumerate(self.data_loader_train):
                ...
                features_teacher = extract_features(self.model_student, 1)(data)
                features_student = extract_features(self.model_student, 1)(data)

                # student与teacher feature之间的距离
                loss_feature = CRD(output_teacher, output_student.detach(), temperature=T)

                #自蒸馏loss
                loss_mse = nn.MSELoss()(features_student, features_student)

                loss_ce = self.criterion(output_student,label)

                loss = alpha*loss_feature + (1-alpha)*loss_ce+0.3*loss_mse

                train_loss_crd.append(loss.item())
            ...
            logger.info(f'[{self.args.n_epoch_s}/{epoch}] <{len(self.data_loader_train)}/{step}>, loss: {loss.item()}')
            macc = acc_group / data_count
            train_acc_crd.append(macc)

            if (epoch + 1) % self.args.val_freq == 0:
                acc, loss = self.val_student()
                val_acc_crd.append(acc)
                val_loss_crd.append(loss.item())
                if np.sum(acc) >= np.sum(self.best_acc):
                    self.best_acc = acc
                    torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_crd, 'best_model.pth'))

            if (epoch + 1) % self.args.save_freq == 0:
                torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_crd, f'{epoch}.pth'))
                # plot loss and accuracy
            plot_loss_and_acc(train_loss_crd, train_acc_crd,val_loss_crd, val_acc_crd,dir=args.save_dir_crd)

            save_metrics_to_excel(train_loss_crd, train_acc_crd,val_loss_crd, val_acc_crd, self.args.save_dir_crd)

if __name__ == '__main__':
    args=make_args()
    trainer=Trainer(args)
    trainer.train()
    trainer.distillation_crd_self(T=5, alpha=0.6)