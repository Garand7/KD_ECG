...
def make_args():
    ...
    parser.add_argument('--save_dir', type=str, default='output_m_nst', help='architecture name')
    parser.add_argument('--save_dir_nst', type=str, default='output_s_nst_75k', help='architecture name') #output_s_nst_175
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    parser.add_argument('--lr_nst', type=float, default=2e-4, help='learning rate')
...
class Trainer:
    def __init__(self, args):
        ...
        os.makedirs(args.save_dir_nst, exist_ok=True)
    ...
    def build_model(self):
        ...
        self.optimizer_student = torch.optim.AdamW(self.model_student.parameters(), lr=self.args.lr_nst)
    ...
    def distillation_nst(self, T, alpha):
        self.best_acc = 0
        train_acc_nst =[]
        train_loss_nst = []
        val_acc_nst = []
        ...
        for epoch in range(args.n_epoch_s):
            ...
            for step, (data, label) in enumerate(self.data_loader_train):
                ...
                nstLOSS = NSTLoss()
                loss_nst = nstLOSS(output_student, output_teacher)

                loss = alpha*loss_nst+ (1-alpha)*(loss_ce+loss_kd)
                train_loss_nst.append(loss.item())
            ...
            logger.info(f'[{self.args.n_epoch_s}/{epoch}] <{len(self.data_loader_train)}/{step}>, loss: {loss.item()}')
            macc = acc_group / data_count
            train_acc_nst.append(macc)

            torch.cuda.empty_cache()

            if (epoch + 1) % self.args.val_freq == 0:
                acc, loss = self.val_student()
                val_acc_nst.append(acc)
                val_loss_nst.append(loss.item())
                if np.sum(acc) >= np.sum(self.best_acc):
                    self.best_acc = acc
                    torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_nst, 'best_model.pth'))

            if (epoch + 1) % self.args.save_freq == 0:
                torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_nst, f'{epoch}.pth'))
                # plot loss and accuracy
            plot_loss_and_acc(train_loss_nst, train_acc_nst,val_loss_nst, val_acc_nst,dir=args.save_dir_nst)

            save_metrics_to_excel(train_loss_nst, train_acc_nst,val_loss_nst, val_acc_nst, self.args.save_dir_nst)

if __name__ == '__main__':
    args=make_args()
    trainer=Trainer(args)
    trainer.train()
    trainer.distillation_nst(T=5,alpha=0.6)