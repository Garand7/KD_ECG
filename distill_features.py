...
def make_args():
    ...
    parser.add_argument('--save_dir', type=str, default='output_m_fitnet', help='architecture name')
    parser.add_argument('--save_dir_fitnet', type=str, default='output_s_fitnet_75k', help='architecture name') #output_s_fitnet_175
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    parser.add_argument('--lr_fitnet', type=float, default=2e-4, help='learning rate')
...
#
# # 随机提取一段信号
# tmodel = unet_m(1,3).to(device)
# smodel = unet_s(1,3).to(device)
#
...
# #%%
# a = extract_features(tmodel,1)
# #%%
# b = extract_features(smodel,1)
# #%%
# features_t = a(data)
# #%%
# features_s = b(data)
...
class Trainer:
    def __init__(self, args):
        ...
        os.makedirs(args.save_dir_fitnet, exist_ok=True)
    ...
    def build_model(self):
        ...
        self.optimizer_student = torch.optim.AdamW(self.model_student.parameters(), lr=self.args.lr_fitnet)
    ...
    #     # 提取中间层特征
    #     self.features_extract = nn.Sequential(*list(model.children())[:1])
    ...
    def distillation_fitnet(self, T , alpha):
        self.best_acc = 0
        train_acc_fitnet=[]
        train_loss_fitnet = []
        val_acc_fitnet = []
        ...
        for epoch in range(args.n_epoch_s):
            ...
            for step, (data, label) in enumerate(self.data_loader_train):
                ...
                loss_mse = nn.MSELoss()(features_student,features_teacher)

                loss = (1-alpha)*loss_ce+alpha*loss_mse
                train_loss_fitnet.append(loss.item())
            ...
            logger.info(f'[{self.args.n_epoch_s}/{epoch}] <{len(self.data_loader_train)}/{step}>, loss: {loss.item()}')
            macc = acc_group / data_count
            train_acc_fitnet.append(macc)

            if (epoch + 1) % self.args.val_freq == 0:
                acc, loss = self.val_student()
                val_acc_fitnet.append(acc)
                val_loss_fitnet.append(loss.item())
                if np.sum(acc) >= np.sum(self.best_acc):
                    self.best_acc = acc
                    torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_fitnet, 'best_model.pth'))

            if (epoch + 1) % self.args.save_freq == 0:
                torch.save(self.model_student.state_dict(), os.path.join(self.args.save_dir_fitnet, f'{epoch}.pth'))
                # plot loss and accuracy
            plot_loss_and_acc(train_loss_fitnet, train_acc_fitnet , val_loss_fitnet , val_acc_fitnet,dir=args.save_dir_fitnet)

            save_metrics_to_excel(train_loss_fitnet, train_acc_fitnet , val_loss_fitnet , val_acc_fitnet, self.args.save_dir_fitnet)

if __name__ == '__main__':
    args=make_args()
    trainer=Trainer(args)
    trainer.train()
    trainer.distillation_fitnet(T=3,alpha=0.5)
