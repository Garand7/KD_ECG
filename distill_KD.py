...
def make_args():
    ...
    parser.add_argument('--save_dir', type=str, default='output_m_500_distill_only', help='architecture name')
    parser.add_argument('--save_dir_kd', type=str, default='output_s_75k', help='architecture name')  #output_s_500
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    parser.add_argument('--lr_kd', type=float, default=2e-4, help='learning rate')
...
class Trainer:
    ...
    def distillation_kd(self, T , alpha):
        ...
        for epoch in range(args.n_epoch_s):
            ...
            logger.info(f'[{self.args.n_epoch_s}/{epoch}] <{len(self.data_loader_train)}/{step}>, loss: {loss.item()}')
            macc = acc_group / data_count
            train_acc_kd.append(macc)

            if (epoch + 1) % self.args.val_freq == 0:
                acc, loss = self.val_student()
                val_acc_kd.append(acc)
            ...
            if (epoch + 1) % self.args.save_freq == 0:
                ...
                # plot loss and accuracy
            plot_loss_and_acc(train_loss_kd, train_acc_kd,val_loss_kd, val_acc_kd,dir=args.save_dir_kd)
            # 保存数据
            save_metrics_to_excel(train_loss_kd, train_acc_kd, val_loss_kd, val_acc_kd, self.args.save_dir_kd)

if __name__ == '__main__':
    args=make_args()
    trainer=Trainer(args)
    trainer.train()
    trainer.distillation_kd(T=3, alpha=0.6)